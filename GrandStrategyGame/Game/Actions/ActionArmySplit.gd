...
func _init(
	army_: Army = null,
	province_: Province = null,
	troop_partition_: PackedInt32Array = [],
):
	army = army_
	province = province_
...
func play_action():
	var number_of_clones: int = troop_partition.size() - 1
	for i in number_of_clones:
		...
		var army_clone: Army = army.duplicate(7) as Army
		...
		armies_node.add_army(army_clone)
