@tool
class_name ItemRangeIntNode
extends PropertyTreeNode
## A [PropertyTreeNode] that displays an [ItemRangeInt].
## Note that the item's text property is not used.

@export var item: ItemRangeInt
...
func _on_from_spin_box_value_changed(value: float) -> void:
	item.min_value = roundi(value)


func _on_to_spin_box_value_changed(value: float) -> void:
	item.max_value = roundi(value)


func _on_item_value_changed(new_value: Vector2i) -> void:
