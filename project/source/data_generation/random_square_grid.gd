class_name RandomSquareGrid
extends GameGeneration
## Takes given JSON data and populates it with a valid square  map,
## adds in some countries and places those countries on the map.
##
## See also: [SquareGridGeneration]
...
func apply(raw_data: Dictionary) -> void:
	SquareGridGeneration.new().apply(raw_data, grid_width, grid_height)
