...
func actions(game: Game, player: GamePlayer) -> Array[Action]:
	...
	for province in frontline_provinces:
		...
		var army_size: int = _army_size(
				armies.dictionary[province.id].ordered_list,
				true,
		...
		for linked_province in game.world.provinces.links_of(province.id):
			...
			var enemy_army_size: int = _army_size(
					armies.dictionary[linked_province.id].ordered_list,
					false,
					player.playing_country
			)
	...
	for army in (
		...
		var province: Province = army.province(game.world.provinces)
		...
		if frontline_provinces.has(province):
			...
			if hostile_links.size() == 1:
				...
				var hostile_army_size: int = _hostile_army_size(
						army.owner_country,
						armies.dictionary[hostile_links[0].id].ordered_list
				)
			...
			# Otherwise, we might need to keep some troops at home to defend.
				...
				for hostile_link in hostile_links:
					...
					var hostile_army_size: int = _hostile_army_size(
							army.owner_country,
							armies.dictionary[hostile_link.id].ordered_list
					)
		...
		if _is_province_pathfinding_worth_it(game, player):
			_move_towards_frontlines_pathfinding(
					result,
					army.id,
					army.province(game.world.provinces),
					danger_levels,
					pathfinding
			)
		else:
...
func _move_towards_frontlines(
	...
	frontline_calculation.calculate(
			army.province_id(), provinces, province_filter
	)
...
func _move_towards_frontlines_pathfinding(
	...
	result.append(ActionArmyMovement.new(army_id, target_province.id))


func _army_size(
		army_list: Array[Army],
		is_yours: bool,
		playing_country: Country
) -> int:
	var output: int = 0
